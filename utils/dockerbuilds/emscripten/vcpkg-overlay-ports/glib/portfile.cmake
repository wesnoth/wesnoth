...
if(VCPKG_TARGET_IS_EMSCRIPTEN)
    list(APPEND GLIB_PATCHES emscripten-skip-resolver-probe.patch)
    list(APPEND GLIB_PATCHES emscripten-compat.patch)
endif()
...
if(VCPKG_TARGET_IS_EMSCRIPTEN)
    foreach(tool IN LISTS GLIB_TOOLS)
        set(tool_js "${CURRENT_PACKAGES_DIR}/bin/${tool}.js")
        set(tool_wasm "${CURRENT_PACKAGES_DIR}/bin/${tool}.wasm")

        if(EXISTS "${tool_js}")
            file(COPY "${tool_js}" DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}")
            if(EXISTS "${tool_wasm}")
                file(COPY "${tool_wasm}" DESTINATION "${CURRENT_PACKAGES_DIR}/tools/${PORT}")
            endif()

            set(tool_wrapper "${CURRENT_PACKAGES_DIR}/tools/${PORT}/${tool}")
            file(WRITE "${tool_wrapper}" "#!/usr/bin/env sh\nexec node \"$(dirname \"$0\")/${tool}.js\" \"$@\"\n")
            file(CHMOD "${tool_wrapper}" PERMISSIONS
