...
// Exact biased fight, tried first and taken for any fixed-damage combat; unlike the
// convolution above it accounts for death truncation: once a unit can die partway through,
// the strikes after its death never happen, coupling the two hit point distributions.
// Conditional on h hits over n strikes the hits are a uniform h-subset, so strike i hits
// with probability (h - j) / (n - i) given j spent, and a running hit count makes the fight
// exact. Restricted to fixed damage per strike (no slow, drain, petrify or swarm), which
// keeps hit points a function of hits taken; returns false otherwise for the caller to fall
// back on the convolution.
