...
void terrain_palette::setup(const game_config_view& cfg)
{
	...
	t_translation::ter_list items = gui_.map().get_terrain_list();
	...
	for (const t_translation::terrain_code& t : items) {
		...
		const terrain_type& t_info = gui_.map().get_terrain_info(t);
...
{
	const auto& info = gui_.map().get_terrain_info(terrain);
	...
	tooltip_text << gui_.map().get_terrain_editor_string(terrain);
...
{
	const terrain_type& t_info = gui_.map().get_terrain_info(terrain);
	return t_info.id();
}
...
std::string terrain_palette::get_help_string() const
{
	...
	msg << _("Left-click: ") << gui_.map().get_terrain_editor_string(selected_fg_item())	<< " | "
		<< _("Right-click: ") << gui_.map().get_terrain_editor_string(selected_bg_item()) << "\n";
