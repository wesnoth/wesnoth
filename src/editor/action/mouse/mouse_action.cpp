...
editor_action_ptr mouse_action::drag_left(editor_display& /*disp*/,
		int /*x*/, int /*y*/, bool& /*partial*/, editor_action* /*last_undo*/)
{
	return nullptr;
}

editor_action_ptr mouse_action::drag_right(editor_display& /*disp*/,
		int /*x*/, int /*y*/, bool& /*partial*/, editor_action* /*last_undo*/)
{
	return nullptr;
}

editor_action_ptr mouse_action::drag_end_left(
		editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

editor_action_ptr mouse_action::drag_end_right(
		editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

editor_action_ptr mouse_action::up_right(
		editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

editor_action_ptr mouse_action::up_left(
		editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}

editor_action_ptr mouse_action::key_event(
	editor_display& disp, const SDL_Event& event)
{
	...
	|| event.key.keysym.sym == SDLK_DELETE) {
		...
		if (res == 0 && old_id != nullptr) {
			a.reset(new editor_action_starting_position(map_location(), *old_id));
		} else if (res > 0 && (old_id == nullptr || *old_id == std::to_string(res))) {
			a.reset(new editor_action_starting_position(previous_move_hex_, std::to_string(res)));
		}
	}
	return a;
}
...
editor_action_ptr brush_drag_mouse_action::click_left(editor_display& disp, int x, int y)
{
...
editor_action_ptr brush_drag_mouse_action::click_right(editor_display& disp, int x, int y)
{
	map_location hex = disp.hex_clicked_on(x, y);
	previous_drag_hex_ = hex;
...
editor_action_ptr brush_drag_mouse_action::drag_left(editor_display& disp,
		int x, int y, bool& partial, editor_action* last_undo)
{
	return drag_generic<&brush_drag_mouse_action::click_perform_left>(disp, x, y, partial, last_undo);
}

editor_action_ptr brush_drag_mouse_action::drag_right(editor_display& disp,
		int x, int y, bool& partial, editor_action* last_undo)
{
	return drag_generic<&brush_drag_mouse_action::click_perform_right>(disp, x, y, partial, last_undo);
}

editor_action_ptr brush_drag_mouse_action::drag_end(
		editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}
...
editor_action_ptr brush_drag_mouse_action::drag_generic(editor_display& disp, int x, int y, bool& partial, editor_action* last_undo)
{
	map_location hex = disp.hex_clicked_on(x, y);
	move(disp, hex);
	if (hex != previous_drag_hex_) {
		...
		partial = true;
		editor_action_ptr a = (this->*perform_func)(disp, affected_hexes(disp, hex));
		previous_drag_hex_ = hex;
		return a;
	} else {
		return nullptr;
...
editor_action_ptr mouse_action_paint::click_left(editor_display& disp, int x, int y)
{
...
editor_action_ptr mouse_action_paint::click_right(editor_display& disp, int x, int y)
{
	if (has_ctrl_modifier()) {
		map_location hex = disp.hex_clicked_on(x, y);
...
editor_action_ptr mouse_action_paint::click_perform_left(
		editor_display& /*disp*/, const std::set<map_location>& hexes)
{
	if (has_ctrl_modifier()) return nullptr;
	return editor_action_ptr(new editor_action_chain(editor_action_ptr(new editor_action_paint_area(
			hexes, terrain_palette_.selected_fg_item(), has_shift_modifier()))));
}

editor_action_ptr mouse_action_paint::click_perform_right(
		editor_display& /*disp*/, const std::set<map_location>& hexes)
{
	if (has_ctrl_modifier()) return nullptr;
	return editor_action_ptr(new editor_action_chain(editor_action_ptr(new editor_action_paint_area(
...
editor_action_ptr mouse_action_paste::click_left(editor_display& disp, int x, int y)
{
	map_location hex = disp.hex_clicked_on(x, y);
	return editor_action_ptr(new editor_action_paste(paste_, hex));
}

editor_action_ptr mouse_action_paste::click_right(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}
...
editor_action_ptr mouse_action_fill::click_left(editor_display& disp, int x, int y)
{
	map_location hex = disp.hex_clicked_on(x, y);
	if (has_ctrl_modifier()) {
		terrain_palette_.select_fg_item(disp.map().get_terrain(hex));
		...
		editor_action_ptr a(new editor_action_fill(hex, terrain_palette_.selected_fg_item(),
				has_shift_modifier()));
		return a;
	}
}

editor_action_ptr mouse_action_fill::click_right(editor_display& disp, int x, int y)
{
	map_location hex = disp.hex_clicked_on(x, y);
	if (has_ctrl_modifier()) {
		terrain_palette_.select_bg_item(disp.map().get_terrain(hex));
		...
		/** @todo only take the base terrain into account when searching for contiguous terrain when painting base only */
		//or use a different key modifier for that
		editor_action_ptr a(new editor_action_fill(hex, terrain_palette_.selected_bg_item(),
				has_shift_modifier()));
		return a;
	}
}
...
editor_action_ptr mouse_action_starting_position::up_left(editor_display& disp, int x, int y)
{
	...
	if(!player_starting_at_hex || new_player_at_hex != *player_starting_at_hex) {
		...
		a.reset(new editor_action_starting_position(hex, new_player_at_hex));
	}
	else {
		// Erase current starting position
		a.reset(new editor_action_starting_position(map_location(), *player_starting_at_hex));
	}
...
editor_action_ptr mouse_action_starting_position::click_left(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	click_ = true;
	return nullptr;
}

editor_action_ptr mouse_action_starting_position::up_right(editor_display& disp, int x, int y)
{
	map_location hex = disp.hex_clicked_on(x, y);
	auto player_starting_at_hex = disp.map().is_starting_position(hex);
	if (player_starting_at_hex != nullptr) {
		return editor_action_ptr(new editor_action_starting_position(map_location(), *player_starting_at_hex));
	} else {
		return nullptr;
	}
}

editor_action_ptr mouse_action_starting_position::click_right(editor_display& /*disp*/, int /*x*/, int /*y*/)
{
	return nullptr;
}
