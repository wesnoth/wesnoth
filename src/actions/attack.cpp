	...
	, plague_type()
{
	...
	signed int cth = weapon->modified_chance_to_hit(opp.defense_modifier(resources::gameboard->map().get_terrain(opp_loc), weapon));
	...
	, plague_type()
{
	...
	signed int cth = weapon->composite_value(weapon->get_specials("defense"), 100 - opp_terrain_defense);
	cth = weapon->modified_chance_to_hit(cth, true);
