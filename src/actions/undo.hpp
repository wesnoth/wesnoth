...
class undo_list : boost::noncopyable {
	...
	struct undo_action : boost::noncopyable {
		...
		undo_action(const UnitPtr u, const map_location& loc) :
				route(1, loc),
				view_info(new clearer_info(*u))
			{}
...
public:
	...
	void add_dismissal(const UnitPtr u);
	/// Adds a move to the undo stack.
	void add_move(const UnitPtr u,
	              const std::vector<map_location>::const_iterator & begin,
	              const std::vector<map_location>::const_iterator & end,
	              int start_moves, int timebonus=0, int village_owner=-1,
	              const map_location::DIRECTION dir=map_location::NDIRECTIONS);
	/// Adds a recall to the undo stack.
	void add_recall(const UnitPtr u, const map_location& loc,
	                const map_location& from);
	/// Adds a recruit to the undo stack.
	void add_recruit(const UnitPtr u, const map_location& loc,
	                 const map_location& from);
private:
