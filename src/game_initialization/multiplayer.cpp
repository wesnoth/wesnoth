	...
	saved_game& state, bool observe)
{
	...
	switch (res) {
	case mp::ui::PLAY:
		play_game(disp, state, game_config, IO_CLIENT, preferences::login(),
	...
	bool local_players_only = false)
{
	...
	switch (res) {
	case mp::ui::PLAY:
		play_game(disp, state, game_config, IO_SERVER, preferences::login(), false,
			!local_players_only);
		recorder.clear();
	...
	saved_game& state, const commandline_options& cmdline_opts)
{
	...
	for(unsigned int i = 0; i < repeat; i++){
		...
		play_game(disp, state_copy, game_config, IO_SERVER, preferences::login(), false, false);
	}
	recorder.clear();
}
