...
bool lobby_info::process_gamelist_diff_impl(const config& data)
{
	...
	for(config& c : gamelist_.child("gamelist").child_range("game")) {
		...
		if(diff_result == "new" || diff_result == "modified") {
			...
			current_i->second.status = game_info::display_status::UPDATED;
...
void lobby_info::sync_games_display_status()
{
	...
	while(i != games_by_id_.end()) {
		if(i->second.status == game_info::display_status::DELETED) {
			i = games_by_id_.erase(i);
		} else {
			i->second.status = game_info::display_status::CLEAN;
			++i;
		}
	}
