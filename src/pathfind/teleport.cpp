...
void pathfind::teleport_group::get_teleport_pair(teleport_pair& loc_pair, const unit& u, bool ignore_units)
{
	...
	if (u.matches_filter(cfg_.get_children("filter").front(), loc)) {

		scoped_xy_unit teleport_unit("teleport_unit", loc.x, loc.y, *resources::units);

		terrain_filter source_filter(cfg_.get_children("source").front(), *units);
		source_filter.get_locations(reversed_ ? loc_pair.second : loc_pair.first);

		terrain_filter target_filter(cfg_.get_children("target").front(), *units);
	...
	bool see_all, bool ignore_units)
{
	...
	if (u.get_ability_bool("teleport")) {
		...
		for(; it != teleports.end(); ++it) {
			const std::vector<config*> tunnels = (it->first)->get_children("tunnel");

			for(std::vector<config*>::const_iterator tunnel_it = tunnels.begin(); tunnel_it != tunnels.end(); ++tunnel_it){
				config teleport_group_cfg = **tunnel_it;
...
pathfind::manager::manager(const config &cfg) : tunnels_(), id_(lexical_cast_default<int>(cfg["next_teleport_group_id"], 0)) {
	const config::child_list &tunnel_list = cfg.get_children("tunnel");
	for(config::child_list::const_iterator t = tunnel_list.begin();
			t != tunnel_list.end(); ++t) {
