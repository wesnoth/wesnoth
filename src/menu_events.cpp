...
void menu_handler::recall(int side_num, const map_location& last_hex)
{
	...
	const int res = dlg.get_selected_index();
	if (res < 0) {
	...
	if(!pc_.get_whiteboard()
		...
		const bool success = synced_context::run_and_throw(
			"recall", replay_helper::get_recall(recall_list_team[res]->id(), recall_location, recall_from));
