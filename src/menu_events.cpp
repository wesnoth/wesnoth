...
void menu_handler::recruit(int side_num, const map_location& last_hex)
{
	...
	for(const auto& recruit : recruits) {
		...
		const map_location ignored;
		const std::string err_msg = can_recruit(type->id(), side_num, last_hex, ignored);
		if (err_msg.empty()) {
...
{
	const map_location recruited_from = map_location::null_location();
...
void menu_handler::recall(int side_num, const map_location& last_hex)
{
	...
	const int wb_gold = pc_.get_whiteboard() ? pc_.get_whiteboard()->get_spent_gold_for(side_num) : 0;
	int team_recall_cost = current_team.recall_cost();
	const bool recallable = (team_recall_cost <= current_team.gold() - wb_gold);
	if(!recallable) {
		utils::string_map i18n_symbols;
		i18n_symbols["cost"] = std::to_string(team_recall_cost);
		const std::string msg = VNGETTEXT("You must have at least 1 gold piece to recall a unit.",
