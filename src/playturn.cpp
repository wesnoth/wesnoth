                ...
                unit_map& units, turn_info& turn_data, bool browse)
{
	...
	if(!left_button && new_left_button) {
		...
		if(u != units.end() && !browse &&
		   selected_hex == hex && u->second.side() == team_num) {
		...
		else if(!browse && selected_hex.valid() && selected_hex != hex &&
	...
	if(!browse && command == HOTKEY_ENDTURN) {
		recorder.save_game(gameinfo,string_table["auto_save"]);
		recorder.end_turn();
		return true;
	}

	if(!browse && command == HOTKEY_RECRUIT) {
	...
	if(!browse && command == HOTKEY_RECALL) {
	...
	if(!browse && command == HOTKEY_END_UNIT_TURN) {
	...
	if(!browse && command == HOTKEY_REDO && !redo_stack.empty()) {
