...
unsigned ttext::insert_text(const unsigned offset, const std::string& text)
{
	...
	return insert_unicode(offset, utils::string_to_ucs4string(text));
}
...
unsigned ttext::insert_unicode(const unsigned offset, const ucs4_string& unicode)
{
	...
	ucs4_string tmp = utils::string_to_ucs4string(text_);
	tmp.insert(tmp.begin() + offset, unicode.begin(), unicode.begin() + len);

	set_text(utils::ucs4string_to_string(tmp), false);
