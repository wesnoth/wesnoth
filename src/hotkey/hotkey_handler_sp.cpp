...
{
	return playsingle_controller_.is_observer();
}
...
void playsingle_controller::hotkey_handler::whiteboard_toggle()
{
	...
	if(whiteboard_manager_->is_active()) {
		...
		gui()->announce(
			_("Planning mode activated!") + std::string("\n") + VGETTEXT("(press $hotkey to deactivate)", symbols),
			font::NORMAL_COLOR);
	} else {
...
bool playsingle_controller::hotkey_handler::can_execute_command(const hotkey::ui_command& cmd) const
{
	...
	case hotkey::HOTKEY_WML: {
		if(cmd.id.compare(0, prefixlen, wml_menu_hotkey_prefix) == 0) {
			game_events::wmi_manager::item_ptr item
	...
	case hotkey::HOTKEY_UPDATE_SHROUD:
		return !linger() && viewing_team_is_playing() && viewing_team().is_local_human() && !events::commands_disabled
	...
	case hotkey::HOTKEY_TELEPORT_UNIT:
		return !events::commands_disabled && game_config::debug
			&& play_controller_.get_map().on_board(mouse_handler_.get_last_hex())
			&& play_controller_.current_team().is_local();
	...
	case hotkey::HOTKEY_LABEL_TERRAIN: {
		...
		res = !events::commands_disabled && play_controller_.get_map().on_board(mouse_handler_.get_last_hex())
			&& !gui()->shrouded(mouse_handler_.get_last_hex()) && !is_observer() && (!label || !label->immutable());
		break;
	}
	case hotkey::HOTKEY_CONTINUE_MOVE: {
		...
		if((menu_handler_.current_unit().valid()) && (menu_handler_.current_unit()->move_interrupted()))
			return true;
	...
	case hotkey::HOTKEY_REPLAY_RESET:
		return playsingle_controller_.get_replay_controller()
			&& playsingle_controller_.get_replay_controller()->can_execute_command(cmd);
	case hotkey::HOTKEY_REPLAY_EXIT:
		return playsingle_controller_.is_replay()
			&& (!playsingle_controller_.is_networked_mp() || resources::recorder->at_end());
	default:
		return play_controller::hotkey_handler::can_execute_command(cmd);
	}
	return res;
	...
	auto invalid_save_file = [this, filename](std::string msg) {
		if(playsingle_controller_.is_networked_mp()) {
			...
			const int res = gui2::show_message(
				"", msg + _("Do you want to load it anyway?"), gui2::dialogs::message::yes_no_buttons);
