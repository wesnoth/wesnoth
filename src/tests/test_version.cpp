...
{
	const version_info empty;

	BOOST_CHECK( empty == version_info(0, 0, 0) );
	BOOST_CHECK( empty.str() == "0.0.0" );

	const version_info dots1("........");
	const version_info dots2("...hullo");

	BOOST_CHECK( dots1 == empty);
	BOOST_CHECK( dots2.str() == "0.0.0hullo" );

	const version_info canonical("1.2.3");

	BOOST_CHECK( canonical.is_canonical() );

	const version_info canonical_suffixed("1.2.3+dev");

	BOOST_CHECK( canonical_suffixed > canonical );
	BOOST_CHECK( canonical < canonical_suffixed );

	const version_info non_canonical("1.2.3.4.5.7.8.9");

	BOOST_CHECK( !non_canonical.is_canonical() );

	const version_info non_canonical_suffixed("1.2.3.4.5.7.8.9+dev");

	BOOST_CHECK( non_canonical_suffixed > non_canonical );
	BOOST_CHECK( non_canonical < non_canonical_suffixed );

	const version_info right_zero("1.2.0");
	const version_info no_right_zero("1.2");

	BOOST_CHECK( right_zero == no_right_zero );

	const version_info left_zero("0.1.4");
	const version_info no_left_zero("1.4");
	...
	const version_info somewhat_complex("1.5.10-1.6beta2");
