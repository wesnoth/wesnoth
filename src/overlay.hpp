...
struct overlay
{
	...
	{
		if (this->layer != 0) { // Offset layer to match unit layer. So that they behave the same way.
			this->layer += 5; // Number is the dif between "terrain_bg" and "unit_first" layers in drawing_layer.hpp
		}
	}
	...
	float parallax; // Parallax factor for the overlay, (How fast it moves during scrolling, 1.0 = normal, <1.0 = slower, >1.0 = faster).
