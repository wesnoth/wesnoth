...
{
	#if 0
	if(PangoAttrList* markup_list = pango_layout_get_attributes(layout_.get())) {
		attrs.splice_into(markup_list);
	}

	#endif

	// append any manual attributes to those generated by pango_layout_set_markup
	PangoAttrList* markup_list = pango_layout_get_attributes(layout_.get());
	GSList* l = attrs.as_list();
