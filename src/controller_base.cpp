...
void controller_base::long_touch_callback(int x, int y)
{
	if(long_touch_timer_ != 0 && !get_mouse_handler_base().dragging_started()) {
		int x_now;
		int y_now;
		const uint32_t mouse_state = sdl::get_mouse_state(&x_now, &y_now);
		...
		const int dx = x - x_now;
		const int dy = y - y_now;
		const int threshold = get_mouse_handler_base().drag_threshold();
		const bool yes_actually_dragging = dx * dx + dy * dy >= threshold * threshold;
...
void controller_base::handle_event(const SDL_Event& event)
{
	...
	case DOUBLE_CLICK_EVENT:
		{
		const int x = static_cast<int>(reinterpret_cast<std::intptr_t>(event.user.data1));
		const int y = static_cast<int>(reinterpret_cast<std::intptr_t>(event.user.data2));
...
bool controller_base::handle_scroll(int mousex, int mousey, int mouse_flags)
{
	...
	const int scroll_speed = prefs::get().scroll_speed();
	...
	const double scroll_amount = dt.count() * 0.036 * double(scroll_speed);
	...
	const point dist{int(dx), int(dy)};
...
void controller_base::play_slice()
{
	...
	const bool was_scrolling = scrolling_;

	int mousex, mousey;
	const uint8_t mouse_flags = sdl::get_mouse_state(&mousex, &mousey);

	scrolling_ = handle_scroll(mousex, mousey, mouse_flags);

	const map_location highlighted_hex = get_display().mouseover_hex();
...
void controller_base::execute_action(const std::vector<std::string>& items_arg, int xloc, int yloc, bool context_menu)
{
	...
	for(const std::string& item : items_arg) {
		const hotkey::ui_command cmd = hotkey::ui_command(item);
