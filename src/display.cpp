...
namespace
{
	...
	animated<image::locator>::anim_description create_animation_source(const std::vector<std::pair<std::string,
	...
	multihex_geometry calculate_multihex_geometry(
		const display& disp,
	...
	)
	{
		...
		const rect r(
			static_cast<int>(std::ceil(anchor_hex_point.x + ((pixel_offset_x - image_size.x / 2.0) * zoom_factor))),
			static_cast<int>(std::ceil(anchor_hex_point.y + ((pixel_offset_y - image_size.y / 2.0) * zoom_factor))),
			static_cast<int>(std::floor(image_size.x * zoom_factor)),
			static_cast<int>(std::floor(image_size.y * zoom_factor))
		);
		...
		const point image_center_screen(
			anchor_hex_point.x + static_cast<int>((pixel_offset_x * zoom_factor)),
			anchor_hex_point.y + static_cast<int>((pixel_offset_y * zoom_factor))
		);
		const point top_left_hex_screen_pos = disp.get_location_rect(map_location(min_x, min_y)).center();
		const int px_center_x = static_cast<int>(((image_center_screen.x - top_left_hex_screen_pos.x) / zoom_factor) + (hex_size / 2.0));
		const int px_center_y = static_cast<int>(((image_center_screen.y - top_left_hex_screen_pos.y) / zoom_factor) + (hex_size / 2.0));
	...
	* top-left anchor hex is in an even column (per the map's coordinate system). */
	int multihex_offset_correct(const int px_center_y, const int current_x, const int start_x, const int row_height)
	{
...
void display::add_single_hex_overlay(const map_location& loc, overlay&& ov)
{
	...
	const bool is_anim = parse_animation_string(ov, frames);
	const int hex_size = game_config::tile_size;
	...
	// Create image source.
	if(!is_anim) {
		ov.image_static = create_static_image_source(ov.image, map_location{0,0}, center_x_offset, center_y_offset);
	} else { // Animated image.
		animated<image::locator>::anim_description anim_desc =
			create_animation_source(frames, map_location{0,0}, center_x_offset, center_y_offset);
...
void display::add_multihex_overlay(const map_location& loc, overlay&& ov)
{
	...
	multihex_geometry geom = calculate_multihex_geometry(
	...
	if(ov.duration.count() > 0) {
		set_overlay_duration(loc, ov.id, ov.duration); // Children are removed with the parent.
	}
	if(is_anim) {
		ov.is_animated = true;
	}
