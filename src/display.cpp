...
{
	//first, erase the symbols (if any) from the arrow's previous path
	const std::list<map_location> & previous_path = changed.get_previous_path();
	erase_arrow_symbols(&changed, previous_path);

	//place the new symbols for the arrow
	const std::list<arrow_image> & images = changed.get_images();
	foreach(arrow_image image, images)
	{
		const map_location& loc = image.first;
		const surface surface_to_add = image.second;
		std::list<arrow_surface> & surfaces = arrow_symbols[loc];
		surfaces.push_back(arrow_surface(surface_to_add, &changed));
	}
...
{
	erase_arrow_symbols(&deleted, deleted.get_path());
}

void display::erase_arrow_symbols(arrow const* arrow_pointer, const std::list<map_location> & path)
{
	foreach(const map_location& loc, path)
	{
		std::list<arrow_surface> & arrow_surfaces = arrow_symbols[loc];
		foreach(arrow_surface as, arrow_surfaces)
		{
			if (as.second == arrow_pointer)
			{
				arrow_surfaces.remove(as);
			}
		}
	}
}
