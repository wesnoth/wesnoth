...
{
	return *(static_cast<ai::engine_lua*>(lua_touserdata(L, lua_upvalueindex(1))));
}
...
static int cfun_ai_get_suitable_keep(lua_State* L)
{
	...
	if(lua_isuserdata(L, index)) {
		...
		return luaW_type_error(L, 1, "unit");
	}
...
lua_ai_load::lua_ai_load(lua_ai_context& ctx, bool read_only)
	: L(ctx.L)
	, was_readonly(false)
{
	refcount++;
