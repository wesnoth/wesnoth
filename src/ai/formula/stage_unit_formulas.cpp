				...
				if (i->has_priority_formula()) {
					try {
						...
						if(e.filename == "formula")
							e.line = 0;
						fai_.handle_exception( e, "Unit priority formula error for unit: '" + static_cast<std::string const &>(i->type_id()) + "' standing at (" + str_cast(i->get_location().x + 1) + "," + str_cast(i->get_location().y + 1) + ")");
		...
		if( i.valid() ) {
			...
			if (i->has_formula()) {
				try {
					...
					fai_.handle_exception( e, "Unit formula error for unit: '" +static_cast<std::string const &>( i->type_id() ) + "' standing at (" + str_cast(i->get_location().x + 1) + "," + str_cast(i->get_location().y + 1) + ")");
				}
			}
		}
		...
		if( i.valid() ) {
			if (i->has_loop_formula())
			{
				try {
					...
					fai_.handle_exception( e, "Unit loop formula error for unit: '" +static_cast<std::string const &>( i->type_id()) + "' standing at (" + str_cast(i->get_location().x + 1) + "," + str_cast(i->get_location().y + 1) + ")");
				}
			}
		}
	}
