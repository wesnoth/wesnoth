...
attack_result_ptr readwrite_context_impl::execute_attack_action(
		const map_location& attacker_loc, const map_location& defender_loc, int attacker_weapon)
{
	...
	return actions::execute_attack_action(
		get_side(), true, attacker_loc, defender_loc, attacker_weapon, m_aggression, m_advancements);
}

attack_result_ptr readonly_context_impl::check_attack_action(
		const map_location& attacker_loc, const map_location& defender_loc, int attacker_weapon)
{
	unit_map::iterator i = resources::gameboard->units().find(attacker_loc);
	double m_aggression = i.valid() && i->can_recruit() ? get_leader_aggression() : get_aggression();
	...
	return actions::execute_attack_action(
		get_side(), false, attacker_loc, defender_loc, attacker_weapon, m_aggression, m_advancements);
}

move_result_ptr readwrite_context_impl::execute_move_action(
		const map_location& from, const map_location& to, bool remove_movement, bool unreach_is_ok)
{
	return actions::execute_move_action(get_side(), true, from, to, remove_movement, unreach_is_ok);
}

move_result_ptr readonly_context_impl::check_move_action(
		const map_location& from, const map_location& to, bool remove_movement, bool unreach_is_ok)
{
	return actions::execute_move_action(get_side(), false, from, to, remove_movement, unreach_is_ok);
}

recall_result_ptr readwrite_context_impl::execute_recall_action(
...
recruit_result_ptr readwrite_context_impl::execute_recruit_action(
...
void readonly_context_impl::calculate_possible_moves(std::map<map_location, pathfind::paths>& res,
		move_map& srcdst,
		move_map& dstsrc,
		bool enemy,
...
{
	calculate_moves(
		resources::gameboard->units(), res, srcdst, dstsrc, enemy, assume_full_movement, remove_destinations);
}

void readonly_context_impl::calculate_moves(const unit_map& units,
		std::map<map_location, pathfind::paths>& res,
		move_map& srcdst,
		move_map& dstsrc,
		bool enemy,
		bool assume_full_movement,
		...
		bool see_all) const
{
	for(unit_map::const_iterator un_it = units.begin(); un_it != units.end(); ++un_it) {
		...
		if(un_it->incapacitated() || (!assume_full_movement && un_it->movement_left() == 0)) {
			continue;
		}
...
const defensive_position& readonly_context_impl::best_defensive_position(
		const map_location& loc, const move_map& dstsrc, const move_map& srcdst, const move_map& enemy_dstsrc) const
{
	...
	const std::map<map_location, defensive_position>::const_iterator position = defensive_position_cache_.find(loc);
...
engine_ptr readonly_context_impl::get_engine_by_cfg(const config& cfg)
{
	...
	if(eng == engine_factory::get_list().end()) {
		ERR_AI << "side " << get_side() << " : UNABLE TO FIND engine[" << engine_name << "]" << std::endl;
	...
	if(!new_engine) {
		ERR_AI << "side " << get_side() << " : UNABLE TO CREATE engine[" << engine_name << "] " << std::endl;
		DBG_AI << "config snippet contains: " << std::endl << cfg << std::endl;
		return engine_ptr();
	}
...
const std::vector<unit_ptr>& readonly_context_impl::get_recall_list() const
{
	...
	// TODO: Refactor ai so that friend of ai context is not required
	// of recall_list_manager at this line
	return current_team().recall_list().recall_list_;
}
...
const map_location& readonly_context_impl::suitable_keep(
		const map_location& leader_location, const pathfind::paths& leader_paths) const
{
		...
		if(keeps().find(loc) != keeps().end()) {
			...
			if(resources::gameboard->units().count(loc) == 0) {
				if((*best_free_keep == map_location::null_location())
						|| (move_left_at_loc > move_left_at_best_free_keep)) {
					best_free_keep = &loc;
					move_left_at_best_free_keep = move_left_at_loc;
				}
			} else {
				if((*best_occupied_keep == map_location::null_location())
						|| (move_left_at_loc > move_left_at_best_occupied_keep)) {
