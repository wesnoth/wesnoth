	...
	for(const config& ai_configuration : ais.child_range("ai")) {
		...
		if(id.empty()) {
			ERR_AI_CONFIGURATION
				<< "skipped AI config due to missing id. Config contains:" << std::endl
				<< ai_configuration << std::endl;
			continue;
		}

		if(ai_configurations_.count(id) > 0) {
			ERR_AI_CONFIGURATION
				<< "skipped AI config due to duplicate id [" << id << "]. Config contains:" << std::endl
				<< ai_configuration << std::endl;
			continue;
		}
	...
	for(config& aspect_cfg : parsed_cfg.child_range("aspect")) {
		...
		if(!aspect_cfg.child("default")) {
			WRN_AI_CONFIGURATION
				<< "side " << side << ": aspect with id=[" << aspect_cfg["id"]
