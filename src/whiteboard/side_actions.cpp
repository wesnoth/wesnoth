...
namespace
{
	...
	private:
		...
		void check_recruit_recall(const map_location &loc) {
			UnitPtr leader = second_->get_unit();
			if(leader->can_recruit() && can_recruit_on(*leader, loc)) {
				if(UnitPtr backup_leader = find_backup_leader(*leader)) {
...
side_actions::iterator side_actions::bump_earlier(side_actions::iterator position)
{
	...
	UnitPtr previous_ptr = (*previous)->get_unit();
	UnitPtr current_ptr = (*position)->get_unit();
...
void side_actions::raw_turn_shift()
{
	...
	std::set<UnitPtr> lazy_units;
	BOOST_FOREACH(action_ptr const& act, iter_turn(0)) {
		UnitPtr u = act->get_unit();
		if(u) {
			lazy_units.insert(u);
		}
	}
	...
	std::set<UnitPtr>::iterator lazy_end = lazy_units.end();
	iterator itor = end();
	while(itor != begin()) {
		--itor;
		action_ptr act = *itor;
