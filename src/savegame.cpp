...
void update_start_unit_types(config& replay_start, config& cfg)
{
	...
	if(not suffix.empty()) {
		for(config& side : replay_start.child_range("side")) {
			...
			if(side.has_attribute("type"))
				side["type"] = side["type"].str() + suffix;
...
void make_quick_leader_1_6_prestart(config& event)
{
	...
	ss << "                        [clear_variable]\n";
	ss << "                            "
		  "name=\"leaders_to_make_quick[$i].max_moves,leaders_to_make_quick[$i].moves,leaders_to_make_quick[$i].max_"
		  "hitpoints,leaders_to_make_quick[$i].hitpoints\"\n";
	ss << "                        [/clear_variable]\n";
	ss << "                        [unstore_unit]\n";
	ss << "                            variable=\"leaders_to_make_quick[$i]\"\n";
	ss << "                        [/unstore_unit]\n";
			...
			if(event["name"] == "prestart") {
				if(config& fire_event = event.child("fire_event")) {
					if(fire_event["name"] == "place_units") {
						...
						std::string changes[] = {
								"if searched == actual then;if actual:sub(1, #searched) == searched  then",
	...
	if(config& replay = cfg.child("replay")) {
		for(config& command : replay.child_range("command")) {
			...
			if(config& recruit = command.child("recruit")) {
				...
				int offset = 1;
				if(unit_types.find(recruit_type)->genders().size() < 2)
					offset = 0;
	...
	if(config& replay = cfg.child("replay_start")) {
		if(config& era = replay.child("era")) {
			for(config& era_event : era.child_range("event")) {
				if(not era_event.empty())
					replay.add_child("event", era_event);
			}
...
void convert_old_saves_1_07_0_for_BFW1_14(config& cfg)
{
	if(config& replay = cfg.child("replay_start")) {
		...
		std::string changes[] = {"^Voha;^Voa", "^Voh;^Vo", "^Vhms;^Vhha", "^Vhm;^Vhh", "^Vcha;^Vca", "^Vch;^Vc",
				"^Vcm;^Vc", "Ggf^Fms;Gg^Fms", "Ggf^Fet;Gg^Fet", "Ggf^Fp;Gg^Fms", "Ggf^Uf;Gs^Uf", "Ggf;Gg^Efm", "Qv;Mv"};
		std::ostringstream ss;
		std::size_t found;
		ss << "border_size=1\nusage=map\n\n";
		for(std::string& line : utils::split(replay["map_data"].str(), '\n')) {
		...
		for(config& side : replay.child_range("side")) {
			if(side.has_attribute("colour"))
				side["color"] = side["colour"];
		}
	}
			...
			for(config& command : replay.child_range("command")) {
				...
				if(config& recruit = command.child("recruit")) {
					...
					if(!this_recruit.compare(/*pos=*/0, /*len=*/7, "Footpad")
						...
						// Default traits are defined as: {TRAIT_STRONG, TRAIT_QUICK, TRAIT_INTELLIGENT,
						// TRAIT_RESILIENT}
						...
						if(!this_recruit.compare(/*pos=*/0, /*len=*/13, "Merman Hunter"))
						...
						for(const config& random : it_random) {
							...
							if(count == random_number_offset + 1
									&& ((random["value"].to_int() % second_trait_pool) == quick_second_trait)) {
								++matching_traits;
							}
							++count;
						}
					...
					if(!this_recruit.compare(/*pos=*/0, /*len=*/13, "Elvish Archer")) {
						...
						int second_trait_pool = first_trait_pool - 1;
						// Default traits are defined as: {TRAIT_STRONG, TRAIT_QUICK, TRAIT_INTELLIGENT,
						// TRAIT_RESILIENT, TRAIT_DEXTROUS}
						...
						for(const config& random : it_random) {
							if(count == random_number_offset
									&& ((random["value"].to_int() % first_trait_pool) == strong_first_trait)) {
								++matching_traits;
							}
							if(count == random_number_offset + 1
									&& ((random["value"].to_int() % second_trait_pool) == quick_second_trait)) {
...
void convert_old_saves_1_09_0(config& cfg)
{
			...
			if(event["name"] == "prestart") {
				...
				for(config& side : replay.child_range("side")) {
					...
					for(auto it = multiplayer_side_recruits.begin(); it != multiplayer_side_recruits.end(); ++it) {
						LOG_RG << recruit_list[0] << " in  '" << *it << "'? --> "
							   << (it->find(recruit_list[0], 0) != std::string::npos) << "\n";
						if(it->find(recruit_list[0], 0) != std::string::npos)
							break;
						++found;
					}
					...
					if(found < switch_.child_count("case")) {
						...
						for(config& case_ : switch_.child_range("case")) {
							...
							for(config& unit : case_.child_range("unit")) {
								if(unit.has_attribute("id"))
									continue;
								if(unit_types.find(unit["type"])->race()->id() == "undead")
									continue;
	...
	if(not null_controller && underlying_units == 0)
		offset_unit_id = leader_unit_id;
	...
	if(config& replay = cfg.child("replay")) {
		...
		for(config& command : replay.child_range("command")) {
			...
			if(config& start = command.child("start")) {
				...
				if(next_unit_id > 0)
					result["next_unit_id"] = next_unit_id - offset_unit_id;
				else
					result["next_unit_id"] = "";
			...
			if(config& attack = command.child("attack")) {
				...
				if(attack.has_attribute("attacker_type")) {
					...
					if(attack["defender_weapon"].to_int(-1) > -1) {
						...
						if(counter_blow.specials().has_child("plague"))
							resolve_plague = true;
					}
							...
							for(config& random : command.child_range("random")) {
								...
								if(config& results = random.child("results")) {
									...
									if(attack["defender_weapon"].to_int(-1) > -1) {
										...
										attack_type counter_blow
												= p_defender->attacks()[attack["defender_weapon"].to_int()];
										if(counter_blow.num_attacks() > 0) {
											// (problem here: in results unit_hit is always "defender")
											while(counter
													> p_killing_blow->num_attacks() + counter_blow.num_attacks()) {
											...
											if(p_killing_blow->num_attacks() < low_num_attacks)
												low_num_attacks = p_killing_blow->num_attacks();
											if(counter > 2 * low_num_attacks) {
												if(p_killing_blow->num_attacks() == low_num_attacks)
													switch_role = true;
											} else {
												bool first_strike_defender
														= counter_blow.specials().has_child("firststrike");
									...
									// Can't call p_killing_blow->get_special_bool() as it requires
									// display::get_singleton()
									if(p_killing_blow->specials().has_child("plague") && (p_dead->race_id() != "undead")
...
static void convert_old_saves_1_11_0(config& cfg)
{
	...
	for(config& side : replay_start.child_range("side")) {
		...
		if(side.has_attribute("color"))
			if(side["color"].to_int(-1) != -1)
				side["color"] = color[side["color"].to_int() - 1];
			else if(not side.has_attribute("colour"))
				side["color"] = color[side_id - 1];
			else if(side["colour"].to_int(-1) == -1)
				side["color"] = side["colour"];
			else
				side["color"] = color[side["colour"].to_int() - 1];
	}
	if(config& replay = cfg.child("replay")) {
		...
		for(config& command : replay.child_range("command")) {
			...
			if(config& recruit = command.child("recruit")) {
				...
				if(!recruited_unit_race.compare(/*pos=*/0, /*len=*/6, "undead")
					...
					if(random_calls == 0)
						skip_next_random_seed_sync = true; // No actual call to RNG, so don't send next random_seed
				}
			}
			if(config& attack = command.child("attack")) {
				if(attack.has_attribute("attacker_type")) {
					...
					if(resolve_plague) {
						...
						if(command.has_child("checkup")) {
							...
							for(config& result : checkup.child_range("result")) {
								if(result.has_attribute("hits"))
									++counter;
								if(result.has_attribute("random_calls")) {
									if(plague_kill) {
										if(result["random_calls"].to_int(0) < 13)
											break;
	...
	if(config& replay = cfg.child("replay_start")) {
		...
		for(config& event : replay.child_range("event")) {
			if(not event.empty())
				copy_event.add_child("event", event);
		}
