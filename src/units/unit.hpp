...
public:
	...
	{
		const_attack_ptr opp_weapon = attack.shared_from_this();
	 ...
	 * @param loc The unit's location (to resolve [resistance] abilities)
	 */
	int resistance_against(const attack_type& atk, bool attacker, const map_location& loc, const_attack_ptr weapon = nullptr, const_attack_ptr opp_weapon=nullptr) const
	{
		return resistance_against(atk.type(), attacker, loc , weapon, opp_weapon);
	}
...
private:
	...
	bool ability_affects_weapon(const std::string& ability,const config& cfg,const map_location& loc, const_attack_ptr weapon, const_attack_ptr opp_weapon) const;
