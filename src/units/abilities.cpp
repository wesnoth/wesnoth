...
effect::effect(const unit_ability_list& list, int def, const_attack_ptr att, const std::string& type) :
	effect_list_(),
	composite_value_(0)
{

	int value_set = (type == "leadership") ? std::max(list.highest("value").first, 0) + std::min(list.lowest("value").first, 0) : def;
	...
	for (const unit_ability & ability : list) {
		...
		if((type != "illuminates") && cfg.has_attribute("max_value")){
			max_value = max_value ? std::max(*max_value, cfg["max_value"].to_int()) : cfg["max_value"].to_int();
		}
		if((type != "illuminates") && cfg.has_attribute("min_value")){
	...
	if((type != "leadership") && set_effect_max.type != NOT_USED) {
	...
	if(max_value && min_value) {
		composite_value_ = std::clamp(*min_value, *max_value, static_cast<int>((value_set + addition) * multiplier / divisor));
	} else if(max_value) {
		composite_value_ = std::min(*max_value, static_cast<int>((value_set + addition) * multiplier / divisor));
	} else if(min_value) {
		composite_value_ = std::max(*min_value, static_cast<int>((value_set + addition) * multiplier / divisor));
	} else {
