...
bool unit::ability_active(const std::string& ability,const config& cfg,const map_location& loc) const
{
	...
	if(auto afilter = cfg.child("filter"))
...
{
	auto filter = cfg.child("filter_self");
...
bool unit::ability_affects_weapon(const config& cfg, const_attack_ptr weapon, bool is_opp) const
{
	...
	auto filter = cfg.child(filter_tag_name);
	if(!weapon) {
		return false;
	}
...
namespace { // Helpers for attack_type::special_active()
		                             ...
		                             const std::string & child_tag)
	{
		...
		auto filter_child = filter.child(child_tag);
		if ( !filter_child )
		...
		if(auto filter_weapon = filter_child->child("filter_weapon") ) {
...
{
	if(auto apply_filter = cfg.child("filter_base_value")) {
