...
void unit_ability_t::do_compat_fixes(config& cfg, const std::string& tag, bool inside_attack)
{
	...
	if (!cfg["overwrite_specials"].blank() || cfg.optional_child("overwrite")) {
		deprecated_message("overwrite_specials= or [overwrite] in weapon specials", DEP_LEVEL::INDEFINITE, "", "Use priority with [erase_lower_priority] instead.");
	}
}
...
bool attack_type::affect_self_opponent(const config& overwrite_filter, const active_ability& i) const
{
	auto ctx = fallback_context();
	auto [self, other] = context_->self_and_other(*this);
	const map_location& loc = i.student_loc;
...
namespace
{
	bool priority_checking(active_ability_list& overwriters, const active_ability& i, const const_attack_ptr& att)
	{
		if(overwriters.empty()){
			return false;
		}
		...
		for(const auto& j : overwriters) {
			...
			if(overwrite_filter && j.ability().priority() > ab.priority()) {
				...
				if(att) {
					affect_side = att->affect_self_opponent(*overwrite_filter, i);
				}
	...
	void edit_list(active_ability_list& temp_list, const const_attack_ptr& att)
	{
		...
		utils::erase_if(temp_list, [&](const active_ability& i) {
			return (priority_checking(temp_list, i, att));
		});
	}
}
