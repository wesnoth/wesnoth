	...
	boost::dynamic_bitset<>* active_list) const
{
	...
	for(const auto [key, cfg] : self_->abilities().all_children_view()) {
		if(check_self_abilities_impl(shared_from_this(), other_attack_, cfg, self_, self_loc_, AFFECT_SELF, key, false, true)) {
	...
	for(const unit& u : get_unit_map()) {
		...
		for(const auto [key, cfg] : u.abilities().all_children_view()) {
			if(check_adj_abilities_impl(shared_from_this(), other_attack_, cfg, self_, u, distance, dir, self_loc_, from_loc, AFFECT_SELF, key, false, true)) {
				bool active = !active_list || special_active(cfg, AFFECT_SELF, key);
				const std::string name = cfg["name_affected"];
				const std::string desc = cfg["description_affected"];
...
bool attack_type::check_self_abilities_impl(const const_attack_ptr& self_attack, const const_attack_ptr& other_attack, const config& special, const unit_const_ptr& u, const map_location& loc, AFFECTS whom, const std::string& tag_name, bool leader_bool, bool in_tooltip)
{
	...
	if(abilities_list::all_weapon_tags().count(tag_name) != 0){
		if(u->get_self_ability_bool(special, tag_name, loc) && special_active_impl(self_attack, other_attack, special, whom, tag_name, true, in_tooltip)) {
			return true;
		}
	}
	return false;
...
bool attack_type::check_adj_abilities_impl(const const_attack_ptr& self_attack, const const_attack_ptr& other_attack, const config& special, const unit_const_ptr& u, const unit& from, std::size_t dist, int dir, const map_location& loc, const map_location& from_loc, AFFECTS whom, const std::string& tag_name, bool leader_bool, bool in_tooltip)
{
	...
	if(abilities_list::all_weapon_tags().count(tag_name) != 0) {
		if(u->get_adj_ability_bool(special, tag_name, dist, dir, loc, from, from_loc) && special_active_impl(self_attack, other_attack, special, whom, tag_name, true, in_tooltip)) {
			return true;
		}
	}
	return false;
 ...
 * @param in_tooltip  when true, the opponents's filters and active_on attribute are ignored.
 */
	...
	bool in_tooltip)
{
	...
	// if in_tooltip is true, ignore this step for abilities weapon name_affected displayed same when inactive.
	const std::string & active_on = special["active_on"];
	if(!in_tooltip && !active_on.empty()) {
	...
	if (!in_tooltip && !special_unit_matches(other, self, other_loc, other_attack, special, is_for_listing, "filter_opponent", opp_check_if_recursion))
		return false;
	...
	if ((!in_tooltip || (in_tooltip && is_attacker)) && !special_unit_matches(att, def, att_loc, att_weapon, special, is_for_listing, "filter_attacker", att_check_if_recursion))
		return false;
	...
	if ((!in_tooltip || (in_tooltip && !is_attacker)) && !special_unit_matches(def, att, def_loc, def_weapon, special, is_for_listing, "filter_defender", def_check_if_recursion))
		return false;
