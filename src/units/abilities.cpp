...
unit_ability_list unit::get_abilities(const std::string& tag_name,
		...
		const_attack_ptr opp_weapon) const
{
		...
		boost::dynamic_bitset<>* active_list) const
{
	...
	for(const config::any_child& ab : this->abilities_.all_children_range()) {
		if(!active_list || ability_active(ab.key, ab.cfg, loc_)) {
			const t_string& name = gender_value(ab.cfg, gender_, "name", "female_name", "name").t_str();

			if(!name.empty()) {
				res.emplace_back(ab.cfg["id"], ab.cfg["name"].t_str(), name, ab.cfg["description"].t_str());
			...
			const config::attribute_value& inactive_value =
				gender_value(ab.cfg, gender_, "name_inactive", "female_name_inactive", "name_inactive");
			const t_string& name = !inactive_value.blank()
				? inactive_value.t_str()
				: gender_value(ab.cfg, gender_, "name", "female_name", "name").t_str();

			if(!name.empty()) {
				res.emplace_back(ab.cfg["id"],
					default_value(ab.cfg, "name_inactive", "name").t_str(), name,
		...
		const std::string& ability, const config& cfg, int dir, const map_location& loc, const unit& from) const
{
	...
	for(const config& i : cfg.child_range("affect_adjacent")) {
		...
		if(!filter || unit_filter(vconfig(filter)).set_use_flat_tod(illuminates).matches(*this, loc, from)) {
			return true;
		}
	}
		...
		const TFuncFormula& formula_handler)
{
	return v.apply_visitor(make_get_ability_value_visitor(def, [&](const std::string& s) {
		try {
			...
			lg::wml_error() << "Formula error in ability or weapon special: " << e.type << " at " << e.filename << ':'
							<< e.line << ")\n";
			return def;
		}
	}));
...
std::pair<int, map_location> unit_ability_list::get_extremum(const std::string& key, int def, const TComp& comp) const
{
	...
	for(const unit_ability& p : cfgs_) {
		int value = get_single_ability_value((*p.first)[key], def, p.second, loc(),
 ...
 *           false otherwise.
 */
bool get_special_children(
		std::vector<const config*>& result, const config& parent, const std::string& id, bool just_peeking = false)
{
 ...
 * c.f. swarm_blows()
 */
void attack_type::modified_attacks(bool is_backstab, unsigned& min_attacks, unsigned& max_attacks) const
{
	// Apply [attacks].
 ...
 *                              for elsewhere)
 */
bool attack_type::special_active(const config& special, AFFECTS whom, bool include_backstab) const
{
	...
	, composite_value_(0)
{
	...
	for(const unit_ability& ability : list) {
		...
		if(!cfg["backstab"].blank()) {
			deprecated_message(
				"backstab= in weapon specials", DEP_LEVEL::PREEMPTIVE, {1, 15, 0}, "Use [filter_adjacent] instead.");
		}
		...
		if(const config::attribute_value* v = cfg.get("value")) {
			int value = get_single_ability_value(*v, def, ability.second, list.loc(),
				[&](const wfl::formula& formula, wfl::map_formula_callable& callable) {
			...
			if(!value_is_set && !cumulative) {
				...
				if(cumulative)
					value_set = std::max<int>(value_set, def);
				if(value > value_set) {
					value_set = value;
					set_effect.set(SET, value, ability.first, ability.second);
				}
		...
		if(const config::attribute_value* v = cfg.get("add")) {
			int add = get_single_ability_value(*v, def, ability.second, list.loc(),
				[&](const wfl::formula& formula, wfl::map_formula_callable& callable) {
					callable.add("base_value", wfl::variant(def));
					return formula.evaluate(callable).as_int();
				}
		...
		if(const config::attribute_value* v = cfg.get("sub")) {
			int sub = -get_single_ability_value(*v, def, ability.second, list.loc(),
				[&](const wfl::formula& formula, wfl::map_formula_callable& callable) {
					callable.add("base_value", wfl::variant(def));
					return formula.evaluate(callable).as_int();
				}
		...
		if(const config::attribute_value* v = cfg.get("multiply")) {
			int multiply = static_cast<int>(
				get_single_ability_value(*v, static_cast<double>(def), ability.second, list.loc(),
					[&](const wfl::formula& formula, wfl::map_formula_callable& callable) {
						callable.add("base_value", wfl::variant(def));
