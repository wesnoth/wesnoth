...
bool unit::get_ability_bool(const std::string& tag_name, const map_location& loc) const
{
	...
	for(const unit& u : units) {
		...
		for(const config& j : u.abilities_.child_range(tag_name)) {
			if(get_dist_ability_bool(j, tag_name, loc, u, u.get_location())){
				return true;
			}
		}
	}
...
unit_ability_list unit::get_abilities(const std::string& tag_name, const map_location& loc) const
{
	...
	for(const unit& u : units) {
		...
		for(const config& j : u.abilities_.child_range(tag_name)) {
			...
			if(get_dist_ability_bool(j, tag_name, loc, u, u_loc)){
				res.emplace_back(&j, loc, u_loc);
			}
		}
	}
...
bool attack_type::has_weapon_ability(const std::string& special, bool special_id, bool special_tags) const
{
	...
	for(const unit& u : units) {
		...
		if (u.incapacitated() || (self_ && &u == self_.get()) || (other_ && &u == other_.get())){
			continue;
		}
		if(self_){
			std::vector<special_match> special_tag_matches_dist;
			std::vector<special_match> special_id_matches_dist;
			get_ability_children(special_tag_matches_dist, special_id_matches_dist, u.abilities(), special, special_id , special_tags);
			if(special_tags){
				for(const special_match& entry : special_tag_matches_dist) {
					if(check_dist_abilities(*entry.cfg, entry.tag_name, u, from_loc)){
						return true;
					}
				}
			}
		...
		if(other_){
			...
			if(special_tags){
				for(const special_match& entry : special_tag_matches_odist) {
					if(check_dist_abilities_impl(other_attack_, shared_from_this(), *entry.cfg, other_, u, other_loc_, from_loc, AFFECT_OTHER, entry.tag_name)){
						return true;
					}
				}
			}
	...
	for(const unit& u : units) {
		...
		if(self_){
			for(const auto [key, cfg] : u.abilities().all_children_view()) {
				if(u.ability_matches_filter(cfg, key, filter) && check_dist_abilities(cfg, key, u, from_loc)){
					return true;
				}
			}
		}
		if(other_){
			for(const auto [key, cfg] : u.abilities().all_children_view()) {
				if(u.ability_matches_filter(cfg, key, filter) && check_dist_abilities_impl(other_attack_, shared_from_this(), cfg, other_, u, other_loc_, from_loc, AFFECT_OTHER, key)){
					return true;
				}
			}
		}
