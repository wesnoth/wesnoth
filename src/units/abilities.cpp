	...
	composite_value_(0)
{
	...
	for (const unit_ability & ability : list) {
		...
		if (!cfg["backstab"].blank()) {
			deprecated_message("backstab= in weapon specials", DEP_LEVEL::PREEMPTIVE, {1, 15, 0}, "Use [filter_opponent] with formula instead; the code can be found in data\core\macros\abilities.cfg in WEAPON_SPECIAL_BACKSTAB macro.");
		}
