...
bool unit::get_ability_bool(const std::string& tag_name, const map_location& loc) const
{
	...
	for(const unit& u : units) {
		if(!u.has_ability_distant() || u.incapacitated() || same_unit(u) || !u.affect_distant(tag_name)) {
			continue;
		}
...
static bool ability_active_adjacent_helper(const unit& self, bool illuminates, const config& cfg, const map_location& loc, bool in_abilities_tag)
{
	...
	for(const config &i : cfg.child_range(filter_adjacent)) {
		...
		for(const unit& u : units) {
			const map_location& from_loc = u.get_location();
			std::size_t distance = distance_between(from_loc, loc);
			if(self.same_unit(u) || distance > radius || !ufilt(u, self)) {
				continue;
			}
			int dir = 0;
...
std::vector<std::string> unit::halo_or_icon_abilities(const std::string& image_type) const
{
	...
	for(const unit& u : units) {
		if(!u.has_ability_distant_image() || u.incapacitated() || same_unit(u)) {
			continue;
		}
		const map_location& from_loc = u.get_location();
	...
	boost::dynamic_bitset<>* active_list) const
{
	...
	for(const unit& u : get_unit_map()) {
		if(!u.has_ability_distant() || u.incapacitated() || self_->same_unit(u)) {
			continue;
		}
		const map_location& from_loc = u.get_location();
	...
	bool leader_bool)
{
	...
	if(self){
		for(const unit& u : units) {
			if(!u.has_ability_distant() || u.incapacitated() || self->same_unit(u)) {
				continue;
			}
			const map_location& from_loc = u.get_location();
...
bool attack_type::has_special_or_ability(const std::string& special) const
{
	...
	if(self_) {
		...
		for(const unit& u : units) {
			if(!u.affect_distant(special) || u.incapacitated() || self_->same_unit(u)) {
				continue;
			}
			const map_location& from_loc = u.get_location();
			std::size_t distance = distance_between(from_loc, self_loc_);
	...
	if(other_) {
		...
		for(const unit& u : units) {
			if(!u.affect_distant(special) || u.incapacitated() || other_->same_unit(u)) {
				continue;
			}
			const map_location& from_loc = u.get_location();
...
bool attack_type::has_filter_special_or_ability(const config& filter, bool simple_check) const
{
	...
	if(other_) {
		...
		for(const unit& u : units) {
			if(!u.has_ability_distant() || u.incapacitated() || other_->same_unit(u)) {
				continue;
			}
			const map_location& from_loc = u.get_location();
			std::size_t distance = distance_between(from_loc, other_loc_);
