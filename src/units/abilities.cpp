...
std::pair<std::string, int> attack_type::effective_damage_type() const
{
	...
	const std::pair<std::string, int> alternative_type = select_alternative_type(damage_type_list, resistance_list);
...
std::pair<std::string, std::set<std::string>> attack_type::damage_types() const
{
	...
	const std::string replacement_type = select_replacement_type(damage_type_list);
	for(auto& i : damage_type_list) {
		const config& c = *i.ability_cfg;
		if(c.has_attribute("alternative_type")){
...
double attack_type::modified_damage() const
{
	const double damage_value
		= unit_abilities::effect(get_specials_and_abilities("damage"), damage(), shared_from_this())
	...
	for (const unit_ability & ability : list) {
		...
		if(wham != EFFECT_CUMULABLE){
			if (const config::attribute_value *v = cfg.get("value")) {
				const int value = std::round(get_single_ability_value(*v, static_cast<double>(def), ability, list.loc(),
					att, [&](const wfl::formula& formula, wfl::map_formula_callable& callable) {
		...
		if (const config::attribute_value *v = cfg.get("add")) {
			const int add = std::round(get_single_ability_value(*v, static_cast<double>(def), ability, list.loc(), att,
		...
		if (const config::attribute_value *v = cfg.get("sub")) {
			const int sub = -std::round(get_single_ability_value(*v, static_cast<double>(def), ability, list.loc(), att,
				[&](const wfl::formula& formula, wfl::map_formula_callable& callable) {
					callable.add("base_value", wfl::variant(def));
					return std::round(formula.evaluate(callable).as_int());
				}));
		...
		if (const config::attribute_value *v = cfg.get("multiply")) {
			const int multiply
				= std::round(get_single_ability_value(*v, static_cast<double>(def), ability, list.loc(), att,
								 [&](const wfl::formula& formula, wfl::map_formula_callable& callable) {
									 callable.add("base_value", wfl::variant(def));
