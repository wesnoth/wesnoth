...
void unit::set_affect_distant(bool increase)
{
	if(resources::gameboard){
		for(const auto [key, ability] : abilities().all_children_view()) {
			if(ability.has_child("affect_distant")) {
				resources::gameboard->set_affect_distant(key, increase);
				if(ability.has_attribute("halo_image") || ability.has_attribute("overlay_image")){
					resources::gameboard->set_affect_distant_image(increase);
				}
			}
		}
	}
...
{
	if(resources::gameboard) {
		if(!tag_name.empty() && resources::gameboard->affect_distant(tag_name) > 0){
			return true;
		} else if(tag_name.empty() && resources::gameboard->affect_distant_for_filtering() > 0){
			return true;
		}
	}
	return false;
...
namespace
{
	...
	bool affect_distant_image()
	{
		if(resources::gameboard) {
			return resources::gameboard->affect_distant_for_image();
		}
		return false;
	}
}
...
bool attack_type::has_weapon_ability(const std::string& special, bool special_id, bool special_tags) const
{
	...
	if(self_){
		...
		if(self_->affect_distant(special_tags ? special : "")){
			for(const unit& unit_itor : units){
	...
	if(other_){
		...
		if(other_->affect_distant(special_tags ? special : "")){
			for(const unit& unit_itor : units){
