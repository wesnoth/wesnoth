...
void unit_ability_t::do_compat_fixes(config& cfg, const std::string& tag, bool inside_attack)
{
	...
	for (config& filter_adjacent : cfg.child_range("filter_adjacent")) {
		...
		if (!has_count) {
			std::ostringstream ss;
			// This filter couldn't match since 1.13.2, until 1.19.26.
			//
			// 1.19.26's behavior doesn't match SUF's default. SUF would pass if any of adjacent=
			// contained a matching unit, but the compat code here set count= to exactly the number
	...
	for (config& filter_adjacent : cfg.child_range("filter_adjacent_location")) {
		// For the commont implementation of SLF, omitting adjacent or count triggers defaults, but
