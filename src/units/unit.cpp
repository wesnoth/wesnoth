...
{
	std::map<std::string, bool> affect_distant;
	bool has_distant = false;
	bool has_distant_image = false;
	for(const auto [key, ability] : abilities_.all_children_view()) {
		if(ability.has_child("affect_distant")) {
			if(!affect_distant[key]){
				affect_distant[key] = true;
			}
			if(!has_distant){
				has_distant = true;
			}
			if(!has_distant_image && (ability.has_attribute("halo_image") || ability.has_attribute("overlay_image"))){
				has_distant_image = true;
			}
		}
	}
	affect_distant_ = affect_distant;
	has_ability_distant_ = has_distant;
	has_ability_distant_image_ = has_distant_image;
}
