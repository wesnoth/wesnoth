...
struct unit_filter_adjacent : public unit_filter_base
{
	...
	virtual bool matches(const unit_filter_args& args) const override
	{
		...
		for(const unit& u : units) {
			...
			if(args.u.same_unit(u) || distance > radius || !child_.matches(unit_filter_args{u, from_loc, &args.u, args.fc, args.use_flat_tod} )) {
				continue;
			}
			int dir = 0;
		...
		create_attribute(literal["ability_id_active"],
			...
			[](const std::set<std::string>& abilities, const unit_filter_args& args)
			{
				...
				for(const unit& unit : units) {
					if(!unit.has_ability_distant() || unit.incapacitated() || args.u.same_unit(unit)) {
						continue;
					}
