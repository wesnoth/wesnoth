				...
				for(const std::string& ability : abilities) {
					...
					for(const unit& unit : units) {
						...
						for(const ability_match& entry : ability_id_matches_dist) {
							if(args.u.get_dist_ability_bool(*entry.cfg, entry.tag_name, distance, *dir, args.loc, unit, from_loc)) {
								return true;
							}
						}
					}
						...
						if(c.get_parsed_config()["affect_adjacent"].to_bool(true)) {
							for(const unit& unit : units) {
								...
								for(const auto [key, cfg] : unit.abilities().all_children_view()) {
									if(args.u.get_dist_ability_bool(cfg, key, distance, *dir, args.loc, unit, from_loc)) {
										return true;
									}
								}
							}
