...
{
	unit_anim_.resume_animation();

	for(auto& anim : sub_anims_) {
		anim.second.resume_animation();
	}
}
...
void unit_animator::wait_until(const std::chrono::milliseconds& animation_time) const
{
	...
	// Sleep, effectively pausing the game until the animation reaches the target time
	while(true) {
		...
		auto wait_ms = std::chrono::duration_cast<std::chrono::milliseconds>(remaining_ms / speed);

		// Sleep in small slices to allow input processing
		auto sleep_slice = std::clamp(wait_ms, 1ms, 20ms);
...
void unit_animator::resume_animation()
{
	for(const auto& anim : animated_units_) {
		if(anim.my_unit->anim_comp().get_animation()) {
			anim.my_unit->anim_comp().get_animation()->resume_animation();
		}
	}
}
