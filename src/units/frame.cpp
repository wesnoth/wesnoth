...
frame_builder::frame_builder(const config& cfg,const std::string& frame_string)
	...
	, parallax_mult_(cfg[frame_string + "parallax_mult"])
...
frame_builder& frame_builder::parallax_mult(const std::string& parallax_mult)
{
	parallax_mult_ = parallax_mult;
	return *this;
}
...
frame_parsed_parameters::frame_parsed_parameters(const frame_builder& builder, const std::chrono::milliseconds& duration)
	...
	, parallax_mult_(builder.parallax_mult_, duration_)
...
bool frame_parsed_parameters::does_not_change() const
{
	return
		...
		parallax_mult_.does_not_change() &&
...
frame_parameters frame_parsed_parameters::parameters(const std::chrono::milliseconds& current_time) const
{
	...
	result.parallax_mult = parallax_mult_.get_current_element(current_time, 1.0);
...
std::vector<std::string> frame_parsed_parameters::debug_strings() const
{
	...
	if(!parallax_mult_.get_original().empty()) {
		v.emplace_back("parallax_mult=" + parallax_mult_.get_original());
	}
		...
		const frame_parameters& animation_val, const frame_parameters& engine_val) const
{
	...
	const double parallax_ = current_data.parallax_mult;
	...
	if(image_size.x && image_size.y) {
		...
		if(parallax_ != 1.0f) {
			...
			// Calculate the difference from the center of the game area
			const int dx = x - cx;
			const int dy = y - cy;
		...
		const frame_parameters& engine_val) const
{
	...
	result.parallax_mult = current_val.parallax_mult != 1.0 ? current_val.parallax_mult : animation_val.parallax_mult;
