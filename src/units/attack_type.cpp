...
static bool matches_simple_filter(const attack_type & attack, const config & filter, const std::string& tag_name)
{
	...
	if(auto sub_filter_special = filter.optional_child("experimental_filter_special_active")) {
...
bool attack_type::apply_modification(const config& cfg)
{
	...
	auto remove_specials = cfg.optional_child("experimental_remove_specials");
