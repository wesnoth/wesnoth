...
std::unique_ptr<specials_coontext_t> attack_type::fallback_context(const unit_ptr& self) const
{
	if (context_) {
		return nullptr;
	} else {
		...
		bool attacking = true;
		return std::unique_ptr<specials_coontext_t>(new specials_coontext_t(specials_coontext_t::make({ self , loc, shared_from_this() }, {}, attacking)));
	}
}
