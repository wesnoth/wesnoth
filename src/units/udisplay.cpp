		...
		display& disp)
{
	...
	animator.replace_anim_if_invalid(
		temp_unit.get(),
		"movement",
		a, b,
		step_num,
...
unit_mover::unit_mover(const std::vector<map_location>& path, bool animate, bool force_scroll)
	...
	, can_draw_(disp_ && !disp_->video().update_locked() && !disp_->video().faked() && path.size() > 1)
	, animate_(animate)
...
void unit_mover::proceed_to(unit_ptr u, std::size_t path_index, bool update, bool wait)
{
	...
	for(; current_ < path_index; ++current_)
		...
		if(!is_enemy_
			...
			if(!disp_->tile_fully_on_screen(path_[current_]) || !disp_->tile_fully_on_screen(path_[current_ + 1])) {
				...
				disp_->scroll_to_tiles(
					path_.begin() + current_, path_.end(), game_display::ONSCREEN, true, false, 0.0, force_scroll_);
			...
			if(tiles_adjacent(path_[current_], path_[current_ + 1])) {
				wait_until_ = move_unit_between(
					path_[current_],
					path_[current_ + 1],
					temp_unit_ptr_.get_unit_ptr(),
					current_, path_.size() - (current_ + 2),
					animator_,
					*disp_
				);
			} else if(path_[current_] != path_[current_ + 1]) {
				teleport_unit_between(path_[current_], path_[current_ + 1], *temp_unit_ptr_, *disp_);
			}
		}
 ...
 */
void move_unit(
		const std::vector<map_location>& path, unit_ptr u, bool animate, map_location::DIRECTION dir, bool force_scroll)
{
...
void unit_draw_weapon(const map_location& loc,
		unit& attacker,
		...
		unit* defender)
{
	...
	animator.add_animation(&attacker, "draw_weapon", loc, defender_loc, 0, false, "", {0, 0, 0},
		unit_animation::hit_type::HIT, attack, secondary_attack, 0);
	animator.add_animation(defender, "draw_weapon", defender_loc, loc, 0, false, "", {0, 0, 0},
		unit_animation::hit_type::MISS, secondary_attack, attack, 0);

	animator.start_animations();
	animator.wait_for_end();
}

void unit_sheath_weapon(const map_location& primary_loc,
		unit* primary_unit,
		...
		unit* secondary_unit)
{
	...
	unit_animator animator;
	if(primary_unit) {
		animator.add_animation(primary_unit, "sheath_weapon", primary_loc, secondary_loc, 0, false, "", {0, 0, 0},
			unit_animation::hit_type::INVALID, primary_attack, secondary_attack, 0);
	}

	if(secondary_unit) {
		animator.add_animation(secondary_unit, "sheath_weapon", secondary_loc, primary_loc, 0, false, "", {0, 0, 0},
			unit_animation::hit_type::INVALID, secondary_attack, primary_attack, 0);
	}
...
void unit_die(const map_location& loc,
		unit& loser,
		const_attack_ptr attack,
		const_attack_ptr secondary_attack,
		const map_location& winner_loc,
		unit* winner)
{
	display* disp = display::get_singleton();
	if(do_not_show_anims(disp) || disp->fogged(loc) || !preferences::show_combat()) {
	...
	animator.add_animation(&loser, "death", loc, winner_loc, 0, false, "", {0, 0, 0},
		unit_animation::hit_type::KILL, attack, secondary_attack, 0);
		...
		const std::vector<std::string>* extra_hit_sounds)
{
	...
	for(const unit_ability& ability : attacker.get_abilities("leadership")) {
		...
		animator.add_animation(&*leader, "leading", ability.second, att->get_location(), damage, true, "", {0, 0, 0},
	...
	for(const unit_ability& ability : defender.get_abilities("resistance")) {
		...
		animator.add_animation(&*helper, "resistance", ability.second, def->get_location(), damage, true, "", {0, 0, 0},
			hit_type, attack.shared_from_this(), secondary_attack, swing);
	}
...
void unit_healing(unit& healed, const std::vector<unit*>& healers, int healing, const std::string& extra_text)
{
	...
	for(unit* h : healers) {
		...
		animator.add_animation(h, "healing", h->get_location(), healed_loc, healing);
	}

	if(healing < 0) {
		animator.add_animation(&healed, "poisoned", healed_loc, map_location::null_location(), -healing, false,
			number_and_text(-healing, extra_text), {255, 0, 0});
	} else if(healing > 0) {
		animator.add_animation(&healed, "healed", healed_loc, map_location::null_location(), healing, false,
			number_and_text(healing, extra_text), {0, 255, 0});
	} else {
		animator.add_animation(&healed, "healed", healed_loc, map_location::null_location(), 0, false,
			extra_text, {0, 255, 0});
	}
