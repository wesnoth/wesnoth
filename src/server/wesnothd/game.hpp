...
private:
	...
	{
		// At the start of the game it can happen that current_side_index_ is null,
	...
	std::optional<player_iterator> current_player() const
	{
		...
		return sides_.empty() ? std::optional<player_iterator>() : sides_[current_side()];
	}
	...
	*/
	...
	/** Set to true whenever description_ was changed so that it needs to be sent and update to the clients */
