...
void server::handle_join_game(player_iterator player, simple_wml::node& join)
{
	...
	simple_wml::document diff;
	const bool diff1
		= make_change_diff(*games_and_users_list_.child("gamelist"), "gamelist", "game", g->description(), diff);
	const bool diff2
		= make_change_diff(games_and_users_list_.root(), nullptr, "user", player->info().config_address(), diff);
...
void server::handle_player_in_game(player_iterator p, simple_wml::document& data)
{
	...
	if(data.child("snapshot") || data.child("scenario")) {
		...
		if(g.remove_player(p)) {
			...
			simple_wml::document diff;
			const bool diff1
				= make_change_diff(*games_and_users_list_.child("gamelist"), "gamelist", "game", description, diff);
			const bool diff2
				= make_change_diff(games_and_users_list_.root(), nullptr, "user", player.config_address(), diff);
