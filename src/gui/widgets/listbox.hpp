...
public:
	 ...
	 *
	 * @param sorter_id    The ID of the selectable_item header widget to bind to.
	 * @param f            Any callable whose result is sortable.
	 */
	template<typename Func>
	void set_single_sorter(const std::string& sorter_id, const Func& f)
	{
		initialize_sorter(sorter_id, {
	 ...
	 *
	 * @param index           The sort option to set, relative to @ref options_.
	 ...
	 */
	void set_active_sorting_option(std::size_t index, sort_order::type order, bool select_first = false);

	std::pair<std::size_t, sort_order::type> get_active_sorting_option() const;
