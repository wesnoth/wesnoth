...
bool lua_kernel_base::load_string(const std::string& prog, const std::string& name, const error_handler& e_h, bool allow_unsafe)
{
	...
	const int errcode = luaL_loadbufferx(mState, prog.c_str(), prog.size(), name.empty() ? prog.c_str() : name.c_str(), allow_unsafe ? "tb" : "t");
	if (errcode != LUA_OK) {
...
config luaW_serialize_function(lua_State* L, int func)
{
	...
	for(int i = 1; i <= info.nups; i++, lua_pop(L, 1)) {
		const std::string_view name = lua_getupvalue(L, func, i);
		if(name == "_ENV") {
...
bool lua_kernel_base::load_binary(const config& cfg, const error_handler& eh)
{
	...
	if(auto upvalues = cfg.optional_child("upvalues")) {
		...
		const int funcindex = lua_absindex(mState, -1);
		for(int i = 1; i <= info.nups; i++) {
			const std::string_view name = lua_getupvalue(mState, funcindex, i);
...
config lua_kernel_base::run_binary_lua_tag(const config& cfg)
{
	...
	try {
		const error_handler eh = std::bind(&lua_kernel_base::throw_exception, this, std::placeholders::_1, std::placeholders::_2 );
	...
	for(int i = top + 1; i < lua_gettop(mState); i++) {
		const std::string index = std::to_string(i - top);
