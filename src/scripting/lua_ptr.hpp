...
/**
 * Allows creation of lua_ptr<T> instances, but does not affect the lifetime of the T itself. This
 * allows the instance of T to be deleted while the lua_ptr<T>s still exist.
 *
...
public:
	...
	/**
	 * Does a move constructor make sense? The weak_ptrs are pointing to o's shared_ptr's control block,
	 * so to keep existing weak_ptrs valid the existing control block should be used instead of replaced.
	 *
	 * After the move, the existing control block will point to the new T, and o.self_ will be empty (which
	 * is guaranteed by the specification for std::shared_ptr's move assignment operator).
	 */
