...
playmp_controller::playmp_controller(const config& level,
		...
		bool skip_replay, bool blindfold_replay_, bool is_host, const std::string& controller_client_id) :
	playsingle_controller(level, state_of_game, ticks,
		game_config, video, skip_replay || blindfold_replay_, controller_client_id), //this || means that if blindfold is enabled, quick replays will be on.
