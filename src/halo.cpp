...
public:
	int add(int x, int y, const std::string& image, const map_location& loc,
			ORIENTATION orientation=NORMAL, bool infinite=true, float parallax = 1.0f, float z_order = 0.0f, int pixel_offset_x = 0, int pixel_offset_y = 0);
...
halo_impl::effect::effect(int xpos, int ypos,
		...
		float parallax, float z_order, int pixel_offset_x, int pixel_offset_y):
	images_(img),
...
void halo_impl::effect::update()
{
	...
	if(parallax_ != 1.0f) {
		...
		xpos += static_cast<int>(dx * (parallax_ - 1.0f) * zf);
		ypos += static_cast<int>(dy * (parallax_ - 1.0f) * zf);
	}
...
int halo_impl::add(int x, int y, const std::string& image, const map_location& loc,
		ORIENTATION orientation, bool infinite, float parallax, float z_order, int pixel_offset_x, int pixel_offset_y)
{
	...
	haloes.try_emplace(id, x, y, image_vector, loc, orientation, infinite, parallax, z_order, pixel_offset_x, pixel_offset_y);
...
handle manager::add(int x, int y, const std::string& image, const map_location& loc,
		ORIENTATION orientation, bool infinite, float parallax, float z_order, int pixel_offset_x, int pixel_offset_y)
{
	int new_halo = impl_->add(x,y,image, loc, orientation, infinite, parallax, z_order, pixel_offset_x, pixel_offset_y);
