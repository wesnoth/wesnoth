...
bool str_equals_number(std::string_view str, TNum num)
{
	...
	if(ec != std::errc()) {
		// Shouldnt happen by definition of utils::charconv::buffer_size<To>
		ERR_CF << "Error in from_strng verify, buffer not large enough";
		return false;
	}

	return std::string_view(buffer.data(), ptr - buffer.data()) == str;
}
