	...
	for(const color_t& old_c : old_rgb) {
		...
		if(reference_avg && old_avg <= reference_avg) {
			...
			clist.emplace_back(
				...
				std::min<uint32_t>(255u, static_cast<uint32_t>(old_ratio * mid_c.g + (1 - old_ratio) * min_c.g)),
			...
			clist.emplace_back(
				...
				std::min<uint32_t>(255u, static_cast<uint32_t>(old_ratio * mid_c.g + (1 - old_ratio) * max_c.g)),
				std::min<uint32_t>(255u, static_cast<uint32_t>(old_ratio * mid_c.b + (1 - old_ratio) * max_c.b))
			);
		}
	}
