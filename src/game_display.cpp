...
void game_display::draw_hex(const map_location& loc)
{
	...
	if(!is_shrouded && !reach_map_.empty()) {
		const int tint_pref = prefs::get().reach_map_tint_opacity();
		const std::string tint_opacity = std::to_string(tint_pref);

		const int darken_val = static_cast<int>((1.0 - std::exp(-3.0 * (tint_pref / 100.0))) * 50.0); // This formula gives a non-linear scaling of the darken opacity based on the tint opacity, with a maximum of 50% at 100% tint.
		const std::string darken_opacity = std::to_string(darken_val);

		if(reach_map_.find(loc) != reach_map_.end()) {
			const std::string color = prefs::get().reach_map_color();

			// Tile is in the reach map; fill it with the selected color tint.
			...
			// Draw the directional border lines (images) that outline the reachable area.
			if(loc != mouseoverHex_) { // We remove the reachmap border mask of the hovered hex to avoid weird interactions with other visual objects.
			...
			// Tile is outside the reach map; darken it.
			drawing_buffer_add(drawing_layer::reachmap_highlight, loc, [tex = image::get_texture(game_config::shroud_prefix + ".png~O("+darken_opacity+"%)", image::HEXED)](const rect& dest) {
				draw::blit(tex, dest);
			});
		}
	}
