		...
		if (items[i] == "wml") {
			...
			for (itor = gs_wmi.begin(); itor != gs_wmi.end()
				...
				&& (filter_location.empty()
					|| terrain_filter(vconfig(filter_location), map_, status_, tod_manager_, *(status_.teams), units_)(hex))
...
bool play_controller::in_context_menu(hotkey::HOTKEY_COMMAND command) const
{
	...
	case hotkey::HOTKEY_RECALL: {
		...
		unit_map::const_iterator leader = units_.find_leader(player_number_);
