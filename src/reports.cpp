...
static config unit_moves(const reports::context& rc, const unit* u, bool is_visible_unit)
{
	...
	for (const terrain_movement& tm : terrain_moves) {
		...
		color_t color = game_config::red_to_green(movement_red_to_green, true);
...
static int attack_info(const reports::context& rc, const attack_type &at, config &res, const unit &u, const map_location &hex, const unit* sec_u = nullptr, const_attack_ptr sec_u_weapon = nullptr)
{
	...
	{
		auto ctx = specials_coontext_t::make({ u.shared_from_this(), hex, at.shared_from_this() }, { }, attacking);
...

	{
		...
		auto ctx = (sec_u == nullptr) ? specials_coontext_t::make_for_listing({ u.shared_from_this(), hex, at.shared_from_this() }, attacking) :
		specials_coontext_t::make({ u.shared_from_this(), hex, at.shared_from_this() }, { sec_u->shared_from_this(), sec_u->get_location(), sec_u_weapon }, attacking);
	...
	// 'abilities' version of special_tooltips is below.
	{
		...
		auto ctx = (sec_u == nullptr)
			? specials_coontext_t::make({ u.shared_from_this(), hex, at.shared_from_this() }, { }, attacking)
			: specials_coontext_t::make({ u.shared_from_this(), hex, at.shared_from_this() }, { sec_u->shared_from_this(), sec_u->get_location(), sec_u_weapon }, attacking);
...
static config unit_box_at(const reports::context& rc, const map_location& mouseover_hex)
{
	...
	int bonus_lim = generic_combat_modifier(bonus, unit_alignments::type::liminal, false, rc.tod().get_max_liminal_bonus());
