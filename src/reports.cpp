...
static int attack_info(const reports::context& rc, const attack_type &at, config &res, const unit &u, const map_location &hex, const unit* sec_u = nullptr, const_attack_ptr sec_u_weapon = nullptr)
{
...

	{
		...
		const double specials_damage = at.modified_damage();
		...
		std::string const range = string_table["range_" + at.range()];
		std::pair<std::string, std::set<std::string>> const all_damage_types = at.damage_types();
		std::string const type = all_damage_types.first;
		std::set<std::string> const alt_types = all_damage_types.second;
