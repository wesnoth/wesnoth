...
private:
		...
		: name_()
	{
		...
		try {
			// NOTE: the default_locale objects needs to live as least as long as the locale_info object. Otherwise the
...
struct translation_manager
{
		...
		, is_dirty_(true)
	{
		...
		generator_.categories(
			bl::message_facet | bl::information_facet | bl::collation_facet | bl::formatting_facet | bl::convert_facet);
		generator_.characters(bl::char_facet);
#else
		generator_.categories(bl::category_t::message | bl::category_t::information | bl::category_t::collation
	...
	void add_messages_domain(const std::string& domain)
	{
		...
		if(domain.find('/') != std::string::npos) {
			...
			ERR_G << "illegal textdomain name '" << domain << "', skipping textdomain";
			return;
		}
	...
	void update_locale_internal()
	{
		try {
			...
			current_locale_ = std::locale(
				current_locale_, new wesnoth_message_format(current_locale_, loaded_domains_, loaded_paths_));
			...
			LOG_G << "updated locale to '" << current_language_ << "' locale is now '" << current_locale_.name()
				  << "' ( "
				  << "name='" << info.name() << "' country='" << info.country() << "' language='" << info.language()
				  << "' encoding='" << info.encoding() << "' variant='" << info.variant() << "')";
	...
	std::string debug_description()
	{
		...
		if(std::has_facet<bl::info>(current_locale_)) {
			const bl::info& info = std::use_facet<bl::info>(current_locale_);
			res << " locale: (name='" << info.name() << "' country='" << info.country() << "' language='"
				<< info.language() << "' encoding='" << info.encoding() << "' variant='" << info.variant() << "'),";
		}
...
std::string ascii_to_lowercase(const std::string& str)
{
	...
	std::transform(
		str.begin(), str.end(), std::back_inserter(result), [](char c) { return c >= 'A' && c <= 'Z' ? c | 0x20 : c; });
	return result;
}
} // namespace
...
{
	try {
		return std::use_facet<std::collate<char>>(get_manager().get_locale())
...
int icompare(const std::string& s1, const std::string& s2)
{
	...
	try {
#if BOOST_VERSION < 108100
		return std::use_facet<bl::collator<char>>(get_manager().get_locale())
			.compare(bl::collator_base::secondary, s1, s2);
#else
		return std::use_facet<bl::collator<char>>(get_manager().get_locale())
			.compare(bl::collate_level::secondary, s1, s2);
#endif
	} catch(const std::bad_cast&) {
		static bool bad_cast_once = false;
...
std::string strftime(const std::string& format, const std::tm* time)
{
	...
	dummy.imbue(get_manager().get_locale()); // TODO: Calling imbue() with hard-coded locale appears to work with
											 // put_time in glibc, but not with get_locale()...
	// Revert to use of boost (from 1.14) instead of std::put_time() because the latter does not appear to handle locale
	// properly in Linux
